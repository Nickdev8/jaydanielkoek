<script lang="ts">
	...
	const reflectorShader = {
		...
		uniforms: {
			...
			time: { value: 0 },
			distortionStrength: { value: 0.0002 },
			reflectionBrightness: { value: 0.45 },
			reflectionBackgroundColor: { value: new Color('#080c15') }
		},
...
base.rgb = reflectionBackgroundColor + ( base.rgb - reflectionBackgroundColor ) * reflectionBrightness;`
	...
	const reflector = new Reflector(new PlaneGeometry(showcaseReflectorSize, showcaseReflectorSize), {
